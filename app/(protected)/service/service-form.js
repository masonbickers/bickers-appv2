...
/* ------------------------------------------------------------------ */
/*  FULL SERVICE CHECKLIST (trimmed but sensible)                      */
/* ------------------------------------------------------------------ */
...
// 🔑 MUST match book-work.jsx (multi-draft map)
...
export default function ServiceFormScreen() {
  ...
  const formId = params?.id || "default"; // id from /service/service-form/[id]
  ...
  /* ---------------- LOAD DRAFT FOR THIS FORM FROM ASYNC STORAGE ---------------- */
  ...
  /* ---------------- AUTO-SAVE DRAFT LOCALLY (PER FORM ID) ---------------- */

  useEffect(() => {
    const saveDraft = async () => {
      try {
        // derive vehicle bits for the draft
        const vehicleName =
        ...
        const draftToSave = {
          selectedVehicleId,
          vehicleName, // used in book-work
          registration, // used in book-work
          vehicleSearch,
          odometer,
          serviceType,
          serviceDate,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // 🔥 clear only THIS form's draft from the multi-draft store
      try {
        const raw = await AsyncStorage.getItem(SERVICE_DRAFTS_KEY);
        if (raw) {
          ...
          <View style={styles.fieldGroup}>
            ...
            <View style={styles.readonlyField}>
              <Text style={styles.readonlyText}>
                {nextServiceComputed || "Calculated from service date (+12 months)"}
              </Text>
            </View>
          </View>
        </View>
      ...
      <View style={styles.card}>
        {items.map((item) => (
          <ChecklistRow
            ...
            na={!!checkNA[item]}
            rating={
              typeof checkRatings[item] === "number"
                ? checkRatings[item]
                : null
            }
